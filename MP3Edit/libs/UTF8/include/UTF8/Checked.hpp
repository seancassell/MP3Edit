...
namespace utf8 {
	...
	inline output_iterator replace_invalid(octet_iterator start, 
										   octet_iterator end, 
										   output_iterator out) {
		static const uint32_t replacement_marker 
	...
	uint32_t next(octet_iterator& it, octet_iterator end) {
		uint32_t cp = 0;
		internal::utf_error err_code = utf8::internal::validate_next(it, 
																	 end, 
																	 cp);
		switch (err_code) {
			case internal::UTF8_OK:
	...
	u16bit_iterator utf8to16 (octet_iterator start, 
		...
		while (start < end) {
			...
			if (cp > 0xFFFF) {
				...
				*result++ = static_cast<uint16_t>((cp >> 10) 
...
#include "CPP11.hpp" // All projects use C++14 or later, so always include this
//#endif // C++ 11 or later
